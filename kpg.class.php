...
if ( !class_exists('EkaPicasaGallery') ) {
	class EkaPicasaGallery {
		private $username;
		...
		public function __construct($username = null, $showOnlyAlbums)
		{
			...
			{
				if ( $this -> checkError( new WP_Error('ekapicasa_gallery-username-required', '<strong>'.__('Error', 'ekapicasa_gallery').':</strong> '.__('You must go to the admin section and set your Picasa Web Album Username in the Options section.', 'ekapicasa_gallery') ) ) )
				{
					return false;
				}
			}
		...
		private function displayAlbums()
		{
			...
			// Get the XML
			//----------------------------------------
			$data = wp_cache_get('EkaPicasaGallery', 'EkaPicasaGallery');
			if ( false === $data )
			{
				...
				wp_cache_set('EkaPicasaGallery', $data, 'EkaPicasaGallery', $this -> cacheTimeout);
			}
			$xml = @simplexml_load_string($data);
			if ( $xml === false )
			{
				return new WP_Error( 'ekapicasa_gallery-invalid-response', '<strong>'.__('Error', 'ekapicasa_gallery').':</strong> '.__('the communication with Picasa Web Albums didn\'t go as expected. Here\'s what Picasa Web Albums said', 'ekapicasa_gallery').':<br /><br />'.$data );
			}

			//----------------------------------------
			// Prepare some variables
			//----------------------------------------
			$page = isset($_GET['kpgp']) && intval($_GET['kpgp']) > 1 ? intval($_GET['kpgp']) : 1; // kpgp = ekaPicasa Gallery Page
			...
			//----------------------------------------
			print '<div id="eka-album-list" class="row">';
			...
			foreach( $xml -> entry as $album )
			{
				...
				print "<a href='$albumURL'><img src='$thumbURL' height='$thumbH' width='$thumbW' alt='".str_replace("'", "&#39;", $title)."' class='img-thumbnail $class' /></a>";
				...
				{
					print '<div class="kpg-nbPhotos">'.sprintf(__ngettext('%d photo', '%d photos', $nbPhotos, 'ekapicasa_gallery'), $nbPhotos).'</div>';
				}
				print '</div>';
			}
		...
		private function displayPictures($album, $direct = false, $authKey = '')
		{
			...
			// Get the XML
			//----------------------------------------
			$data = wp_cache_get('EkaPicasaGallery_'.$album, 'EkaPicasaGallery');
			if ( false === $data )
			{
				...
				$data = str_replace('gphoto:', 'gphoto_', $data);
				$data = str_replace('media:', 'media_', $data);
				wp_cache_set('EkaPicasaGallery_'.$album, $data, 'EkaPicasaGallery', $this -> cacheTimeout);
			}
			$xml = @simplexml_load_string($data);
			if ( $xml === false )
			{
			...
			if ( !$direct )
			{
				...
				print "<div id='kpg-backLink'><a href='$backURL'>&laquo; ".__('Back to album list', 'ekapicasa_gallery').'</a></div>';
			}
			...
			if ( $this -> config['albumNbPhoto'] == 1 )
			{
				print '<div id="kpg-nbPhotos">'.sprintf(__ngettext('%d photo', '%d photos', $albumNbPhotos, 'ekapicasa_gallery'), $albumNbPhotos).'</div>';
			}
			if ( $this -> config['albumSlideshow'] == 1 )
			{
				print "<div id='kpg-slideshow'><a href='$albumSlideshow'>".__('Slideshow', 'ekapicasa_gallery')."</a></div>";
			}
			print '</div>';
			...
			//----------------------------------------
			print '<div class="row" id="kpg-pictures">';
			$i = -1; $j = -1;
			foreach( $xml->entry as $photo )
			{
				...
				$j++;
					...
					// Set the height of each box at the same
					print "<div class='col-xs-12 col-sm-6 col-md-4 col-lg-3'>";
		...
		private function paginator ($page, $argName, $perPage, $nbItems, $extraArgs = array())
		{
			if ($perPage > 0)
			{
				...
				if ($nbPage > 1)
				{
					...
					print '<nav id="eka-paginator"><ul class="pagination">';
					if ($page == 1) {
						print '<li class="disabled"><a href="#" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>';
					} else {
						$t = add_query_arg($argName, $page - 1, $url);
						print '<li><a href="' . $t .'" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>';
					}
					...
					for($i = 1; $i <= $nbPage; $i++)
					{
						...
						if ($i == $page)
						{
							...
							print " <li class='active'><a href='#'>$i <span class='sr-only'>(current)</span></a></li>";
						}
						else
						{
							print " <li><a href='$pageURL'>$i</a></li>";
						}
					}
					if ($i == $page + 1) {
						print '<li class="disabled"><a href="#" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>';
					} else {
						$t = add_query_arg($argName, $page + 1, $url);
						print '<li><a href="' . $t .'" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>';
					}
					print '</ul></nav>';
				}
			}
