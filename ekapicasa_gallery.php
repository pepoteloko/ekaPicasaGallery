<?php
/*
Plugin Name: ekaPicasa Gallery
Plugin URI: http://www.ekadesign.eu
Description: Display your Picasa Web Galleries in a post or in a page using Bootstrap.
This plugin is based on kPicasaPlugin by Guillaume Hébert (http://www.boloxe.com/techblog/)
This plugin does not include Bootstrap core files. Your theme should provide them.
Version: 0.1
Author: Eka Design S.C.P

Version History
---------------------------------------------------------------------------
* 1.0: Modified the output of the plugin to use Bootstrap.
...
{
	print 'ekaPicasa Gallery requires PHP version 5 or greater. You are running PHP version '.PHP_VERSION;
	exit;
}
...
{
	print '<strong>'.__('Error', 'ekapicasa_gallery').':</strong> '.__('Your WordPress installation is missing some required functions. Please upgrade your WordPress installation.', 'ekapicasa_gallery');
	exit;
}
else
{
	// this way, the plugin can only be activated if all the requirements are met
	register_activation_hook( __FILE__, 'ekapicasa_gallery_activate' );
}

if ( defined('EKAPICASA_GALLERY_FILTER_PRIORITY') )
{
	print '<strong>'.__('Error', 'ekapicasa_gallery').':</strong> '.__('WordPress is trying to include ekaPicasa Gallery more than once. Please check that another plugin is not doing this.', 'ekapicasa_gallery');
	exit;
}

define('EKAPICASA_GALLERY_FILTER_PRIORITY', 9);
define('EKAPICASA_GALLERY_VERSION', '0.1');

global $kpg_config;
$kpg_config = get_option( 'ekapicasa_gallery_config' );

if ( !is_admin() )
{
	add_action('wp_head', 'ekapicasa_gallery_init');
	add_filter('the_content', 'ekapicasa_gallery_load', EKAPICASA_GALLERY_FILTER_PRIORITY);

	wp_enqueue_style('kpicasa', plugins_url('ekapicasa_gallery.css', __FILE__), false, EKAPICASA_GALLERY_VERSION, 'screen');
...
{
	add_action( 'admin_menu', 'ekapicasa_gallery_admin_menu' );
	add_action( 'admin_init', 'ekapicasa_register_settings' );
}

function ekapicasa_gallery_init()
{
	global $kpg_config;
	...
	elseif ( $kpg_config['picEngine'] == 'fancybox' )
	{
		...
		print "		jQuery('a.fancybox-ekapicasa_gallery').fancybox({ 'hideOnContentClick': false });\n";
		print "	});\n";
		print "</script>\n";
	}
}

function ekapicasa_gallery_load ( $content = '' )
{
	$tmp = strip_tags(trim($content));
	$regex = '/^[\s]*ekapicasa_gallery[\s]*(\(.*\))?[\s]*$/m';
	...
	if ( preg_match($regex, $tmp, $matches) )
	{
		...
		ob_start();
		$gallery = new EkaPicasaGallery($username, $showOnlyAlbums);
		$buffer  = ob_get_clean();
...
function ekapicasa_gallery_admin_menu()
{
	...
	add_submenu_page('plugins.php', __('ekaPicasa Gallery Configuration', 'ekapicasa_gallery'), __('ekaPicasa Gallery', 'ekapicasa_gallery'), 'manage_options', dirname(__FILE__).'/param.php');
}

function ekapicasa_register_settings()
{
	register_setting( 'ekapicasa_gallery_config', 'ekapicasa_gallery_config', 'ekapicasa_gallery_config_sanitize' );
}

function ekapicasa_gallery_config_sanitize($input)
{
...
function ekapicasa_gallery_activate() {
	global $kpg_config;
	...
	if ( $kpg_config === false ) {
		...
		add_option( 'ekapicasa_gallery_config', $kpg_config );
